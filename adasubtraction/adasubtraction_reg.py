...
def adaptive_subtraction(data_patched, multiples_patched, num_patches, nwin, solver, nfilt, solver_dict, clipping):
    ...
    if multiples_patched.ndim == 3:
        ...
        Lop = pylops.SecondDerivative(N=(len(non_zero_patches)*nfilt)) # Second derivative operator
        ...
        Data_Stack_NC = np.concatenate((Data_Stack_NC, np.zeros(len(non_zero_patches)*nfilt))) # Pad data with 0
...
def adasubtraction_reg(data, multiples, nfilt, solver, solver_dict, nwin, clipping=False):
    """Applies adaptive subtraction to a shot gather using a joint optimization for all 
    filters of the patches. Adds a regularization to the optimization problem.
